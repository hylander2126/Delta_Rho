...
if ~exist('optiClient', 'var')
    disp('OPTITRACK NOT INITIALIZED!!! Trying to connect ...')
    optitrack_stream_init
    pause(2)
    return;
end

%% Information from OptiTrack
frameOfData = optiClient.getFrame;

nRigidBodies = 0;

%% Get num of rigid bodies
for i=1:3
    temp = frameOfData.RigidBody(i);
    if ~isempty(temp)
...
for n = 1:nRigidBodies
    rigidBody{n} = frameOfData.RigidBody(n); % Getting rigid body 1 information
end

%% RigidBody translation and rotation
global opti_T opti_theta

% ROBOT SHOULD BE RIGID BODY 1

for n = 1:nRigidBodies
    body = rigidBody{n};

    % Translation
    opti_p = double([body.x; body.y; body.z]);
    % Quaternion
    opti_q = double([body.qw; body.qx; body.qy; body.qz]);
    % Rotation Matrix
