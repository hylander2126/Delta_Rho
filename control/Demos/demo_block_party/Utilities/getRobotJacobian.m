...
%     J(:,i) = 2*[-sind(aw(i)); cosd(aw(i)); p(i,1)*cosd(aw(i)) - p(i,2)*sind(aw(i))]; % Original directly from paper
    J(:,i) = -2*[-sind(aw(i)); cosd(aw(i)); p(i,1)*cosd(aw(i)) - p(i,2)*sind(aw(i))]; % Need to flip signs for intuitive keyboard ctrl
end

J = real(J');
% Jacobian needs to be transposed for all other functions. Each row now
% represents a wheel
