...
%     fprintf('%d rigid bodies detected\n',nRigidBodies)
end
...
for n=1:nRigidBodies
    rigid_x(n) = rigidBody{n}.x;
    rigid_y(n) = rigidBody{n}.y;
    rigid_z(n) = rigidBody{n}.z;

    rigid_qw(n) = rigidBody{n}.qw;
    rigid_qx(n) = rigidBody{n}.qx;
    rigid_qy(n) = rigidBody{n}.qy;
    rigid_qz(n) = rigidBody{n}.qz;

    rigid_Translation(:,n) = double([rigid_x(n);rigid_y(n);rigid_z(n)]);
    
    Opti_Q(:,n) = double([rigid_qw(n) rigid_qx(n) rigid_qy(n) rigid_qz(n)]');
    Opti_R(:,:,n) = quaternions2R(Opti_Q(:,n));

    % if testPt2
    %     Opti_theta(n) = -atan(Opti_R(2,1,n)/Opti_R(1,1,n));
    % else
        Opti_theta(n) = -atan2(Opti_R(2,1,n),Opti_R(1,1,n));
    % end
end
