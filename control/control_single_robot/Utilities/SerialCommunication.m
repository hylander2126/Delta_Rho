...
if(code >= 108) % 128) % if no response required
    for n=1:length(robot)
        ...
        % temp = AddressCode % Temp printing the address code

        data = robot(n).(field).data;
        data = robot(n).(field).convertor(data);
        
        for i=1:length(data)
            fwrite(s1,data(i),robot(n).(field).type);
        end
    end
